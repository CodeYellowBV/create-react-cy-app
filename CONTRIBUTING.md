...
1. Clone the repo with `git clone https://github.com/CodeYellowBV/create-react-cy-app`

2. Run `npm install` in the root `create-react-cy-app` folder.
...
```
npm run create-react-cy-app my-app
cd my-app
```
...
3. In most releases, only `react-cy-scripts` needs to be released. If you don’t have any changes to the `packages/create-react-cy-app` folder, you don’t need to bump its version or publish it (the publish script will publish only changed packages).
4. Note that files in `packages/create-react-cy-app` should be modified with extreme caution. Since it’s a global CLI, any version of `create-react-cy-app` (global CLI) including very old ones should work with the latest version of `react-cy-scripts`.
