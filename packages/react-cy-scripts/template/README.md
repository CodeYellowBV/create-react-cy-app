This project was bootstrapped with [Create React App](https://github.com/CodeYellowBV/create-react-cy-app).

Below you will find some information on how to perform common tasks.<br>
You can find the most recent version of this guide [here](https://github.com/CodeYellowBV/create-react-cy-app/blob/master/packages/react-cy-scripts/template/README.md).
...
* `create-react-cy-app` is a global command-line utility that you use to create new projects.
* `react-cy-scripts` is a development dependency in the generated projects (including this one).

You almost never need to update `create-react-cy-app` itself: it delegates all the setup to `react-cy-scripts`.

When you run `create-react-cy-app`, it always creates the project with the latest version of `react-cy-scripts` so you’ll get all the new features and improvements in newly created apps automatically.

To update an existing project to a new version of `react-cy-scripts`, [open the changelog](https://github.com/CodeYellowBV/create-react-cy-app/blob/master/CHANGELOG.md), find the version you’re currently on (check `package.json` in this folder if you’re not sure), and apply the migration instructions for the newer versions.

In most cases bumping the `react-cy-scripts` version in `package.json` and running `npm install` in this folder should be enough, but it’s good to consult the [changelog](https://github.com/CodeYellowBV/create-react-cy-app/blob/master/CHANGELOG.md) for potential breaking changes.

We commit to keeping the breaking changes minimal so you can upgrade `react-cy-scripts` painlessly.
...
* [Decorators](https://github.com/tc39/proposal-decorators) (stage 2 proposal).

Learn more about [different proposal stages](https://babeljs.io/docs/plugins/#presets-stage-x-experimental-presets-).

Make sure that the client is okay with supporting only the browsers that support ES6.
...
-    "start": "react-cy-scripts start",
-    "build": "react-cy-scripts build",
+    "start": "react-cy-scripts start & npm run watch-css",
+    "build": "react-cy-scripts build && npm run build-css",
     "test": "react-cy-scripts test --env=jsdom",
...
>Note: You must create custom environment variables beginning with `CY_FRONTEND_`. Any other variables except `NODE_ENV` will be ignored to avoid accidentally [exposing a private key on the machine that could have the same name](https://github.com/facebookincubator/create-react-app/issues/865#issuecomment-252199527). Changing any environment variables will require you to restart the development server if it is running.

These environment variables will be defined for you on `process.env`. For example, having an environment
variable named `CY_FRONTEND_SECRET_CODE` will be exposed in your JS as `process.env.CY_FRONTEND_SECRET_CODE`.
...
render() {
  return (
    <div>
      ...
      <form>
        <input type="hidden" defaultValue={process.env.CY_FRONTEND_SECRET_CODE} />
      </form>
    </div>
  );
}
...
During the build, `process.env.CY_FRONTEND_SECRET_CODE` will be replaced with the current value of the `CY_FRONTEND_SECRET_CODE` environment variable. Remember that the `NODE_ENV` variable will be set for you automatically.
...
The above form is looking for a variable called `CY_FRONTEND_SECRET_CODE` from the environment. In order to consume this
...
You can also access the environment variables starting with `CY_FRONTEND_` in the `public/index.html`. For example:

```html
<title>%CY_FRONTEND_WEBSITE_NAME%</title>
```

Note that the caveats from the above section apply:

* Apart from a few built-in variables (`NODE_ENV` and `PUBLIC_URL`), variable names must start with `CY_FRONTEND_` to work.
...
```cmd
set CY_FRONTEND_SECRET_CODE=abcdef&&npm start
```
...
```bash
CY_FRONTEND_SECRET_CODE=abcdef npm start
```
...
```
CY_FRONTEND_SECRET_CODE=abcdef
```
...
>[Read the migration guide to learn how to enable it in older projects!](https://github.com/CodeYellowBV/create-react-cy-app/blob/master/CHANGELOG.md#migrating-from-023-to-030)
...
If you run `npm test` and the console gets stuck after printing `react-cy-scripts test --env=jsdom` to the console there might be a problem with your [Watchman](https://facebook.github.io/watchman/) installation as described in [facebookincubator/create-react-app#713](https://github.com/facebookincubator/create-react-app/issues/713).
...
If you have ideas for more “How To” recipes that should be on this page, [let us know](https://github.com/facebookincubator/create-react-app/issues) or [contribute some!](https://github.com/facebookincubator/create-react-app/edit/master/packages/react-cy-scripts/template/README.md)
